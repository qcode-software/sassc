...
install: $(OBJECTS) $(LIBSASS_DIR)/lib/libsass.so
	gcc -O2 -o $(INSTALL_DIR)/$(NAME)  $^

$(LIBSASS_DIR)/lib/libsass.so:
	$(MAKE) -C $(LIBSASS_DIR) $(LIBSASS_DIR)/lib/libsass.so VERSION=$(LIBSASS_VERSION) TMP_DIR=$(LIBSASS_DIR)
